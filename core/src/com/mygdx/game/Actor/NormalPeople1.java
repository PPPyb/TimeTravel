...
public class NormalPeople1 extends NPC {
    ...
    public BodyDef bdef;
    public NormalPeople1(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        frames=new Array<TextureRegion>();
        for(int i=0;i<3;i++)
            frames.add(new TextureRegion(screen.getNormalPeople1Atlas().findRegion("nomalPeople1"),48*i,98,48,49));
        NormalPeopleRightRun=new Animation(0.1f,frames);
        frames.clear();
        for(int i=0;i<3;i++)
            frames.add(new TextureRegion(screen.getNormalPeople1Atlas().findRegion("nomalPeople1"),48*i,49,48,49));
        NormalPeopleLeftRun=new Animation(0.1f,frames);
        frames.clear();
        for(int i=0;i<3;i++)
            frames.add(new TextureRegion(screen.getNormalPeople1Atlas().findRegion("nomalPeople1"),48*i,147,48,49));
        NormalPeopleUpRun=new Animation(0.1f,frames);
        frames.clear();
        for(int i=0;i<3;i++)
            frames.add(new TextureRegion(screen.getNormalPeople1Atlas().findRegion("nomalPeople1"),48*i,0,48,49));
